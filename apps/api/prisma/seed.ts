...
async function main() {
  ...
  for (const child of family.children) {
    await prisma.habitLog.createMany({
      data: [
        { childId: child.id, water: 5, fruits: 2, veggies: 1, activity: 30, screen: 60 },
        { childId: child.id, water: 6, fruits: 3, veggies: 2, activity: 40, screen: 50 }
      ]
    });
  }
  ...
  await prisma.tip.createMany({
    data: [
      { title: 'Hydrate Often', content: 'Drink at least 5 glasses of water daily' },
